...
- **Countermeasure:** Add official TOP/RECENT keyword discovery for six reviewed design topics; keep RECENT discovery-only; allow only taxonomy-mapped TOP results to cross a conservative community floor; count all Threads results as one independent source; require two other source groups before drafting a proposal; keep every proposal review-only.
...
- **Validation performed:** Five Threads-specific regressions cover bounded authenticated requests, sanitization, host filtering, deduplication with TOP preservation, missing/denied credentials, controlled candidate qualification, and single-source independence. The full unit suite passes 28/28, routing passes 10/10, all 17 skills validate, and the configured Design Quality report remains 92/100.
