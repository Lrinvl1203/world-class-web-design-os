...
WDX separates four kinds of evidence so a strong internal result is not presented as market proof.
...
| Internal design | Does an artifact meet the WDX rubric and anti-slop criteria? | Nocturne 92.8; VANTA 91.8 hold | Internal WDX score only |
...
The first public evaluation should use the same brief across WDX and at least two credible alternatives, with identity hidden from reviewers where practical. Record:
...
Nocturne Concierge demonstrates that the system can cross its internal 92-point threshold on an editorial hospitality archetype. VANTA Forge deliberately remains on hold at 91.8, showing that the threshold is enforced rather than rounded into a pass. Neither experiment currently has representative-user or conversion evidence.
