...
Public-discovery sources may configure a case-insensitive `excludeAuthors` list. Maintainer-authored promotion is excluded from public community discovery so the project cannot reinforce its own claims or mistake its distribution activity for external evidence. Authorized owned-post insights remain a separate collector for measured distribution outcomes and still share the single Threads source family.
